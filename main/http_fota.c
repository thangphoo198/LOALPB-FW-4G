...
int ql_fota_firmware_download(int profile_idx, qlFotaDwnldMod mode, char * address, char * username, char * password, qlFotaProgressCB_t cb)
{
	...
	if(mode == QL_FOTA_DWNLD_MOD_HTTP)
	{
	    ...
	    ql_http_client_setopt(fota_httpclient, QL_HTTP_CLIENT_OPT_ASYN, 0); //ͬ��ģʽ
...
#else
#define FOTA_SERVER_ADDRESS 	 "http://103.143.206.116:8001/api/dl/firmware?k=137cf6e6-aee3-11ef-b10b-22265c07037b&s=123456&v=1.1"
...
{
	(void)argv;
    fota_exam_log("========== fota test start 22222222222222222222 ==========");
