...
2023/07/13        Sharon.li  增加example使用说明
===========================================================================================================*/
/**
example 使用说明。注意:R07基线的SDK 为单卡协议栈,因此仅支持双卡单待。R07基线SDK 不再支持 xxx_by_simid()类似函数
1、创建ql_sim_test_ctrl_task，并使用Main UART 作为与上位机的交互串口.
2、通过串口向模块发 SWITCH_TO_1 代表切卡到卡1，代码立即执行切卡操作
3、通过串口向模块发 SWITCH_TO_2 代表切卡到卡2，代码立即执行切卡操作
4、通过串口向模块发 SIM_GET_INFO 代表获取当前SIM的信息
5、通过串口向模块发 START_DATACALL 代表使用当前主卡进行数据拨号
**/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
//is default,Depending on the project. EC600K-GPIO_PIN_NO_85
#define QL_USIM1_DET_PIN  GPIO_PIN_NO_49  
