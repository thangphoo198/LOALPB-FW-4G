...
static void quec_control_uart_callback(QL_UART_PORT_NUMBER_E port, void *para)
{
	...
	if(0 == memcmp(m_RxBuf_Uart1,"QUEC_START_DATACALL",14)) //开启数据拨号
	{
		APP_DEBUG("<-%s -->\r\n",m_RxBuf_Uart1);
	...
	if(0 == memcmp(m_RxBuf_Uart1,"QUEC_START_DATACALL_V6",22)) //开启数据拨号
	{
		APP_DEBUG("<-%s -->\r\n",m_RxBuf_Uart1);
		DATACALL_ACTION_T quecdatacall_msg = {0};
	...
	if(0 == memcmp(m_RxBuf_Uart1,"APN_CHANGE",10)) //APN 发生改变
	{
		APP_DEBUG("<-%s -->\r\n",m_RxBuf_Uart1);
		DATACALL_ACTION_T quecdatacall_msg = {0};
	...
	if(0 == memcmp(m_RxBuf_Uart1,"Stop_DataCall",13)) //停止数据拨号
	{
		APP_DEBUG("<-%s -->\r\n",m_RxBuf_Uart1);
...
static void datacall_test_one(void * argv)
{
	...
	while(1)
	{
		...
		switch(msg.msg)
		{
			...
			case QUEC_START_DATACALL_V6://V6 数据拨号
				printf("Enable data call Sync mode,IPV6, APN Name:NULL\r\n");
				ql_set_data_call_asyn_mode(0, ql_data_call_cb);
